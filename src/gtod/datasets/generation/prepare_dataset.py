...
flags.DEFINE_boolean(
    "delexicalize", False, "Whether to delexicalize non-categorical slots"
)
flags.DEFINE_string("templates_dir", None, "Directory contains utterance templates.")
...
class Preprocessor:
    ...
    def preprocess_schemas(self, schemas_path):
        schemas = json.load(tf.io.gfile.GFile(schemas_path))
        schemas_str_repr = {
            schema["service_name"]: self.preprocess_schema(schema) for schema in schemas
        }
    ...
    def preprocess_schema(self, schema):
        ...
        service_str = (
            f"service_name={schema['service_name']},"
        ...
        with tf.io.gfile.GFile(output_tsv_path, "w") as tsvfile:
            ...
            for dialog in dialogs:
                ...
                for turn in turns:
                    ...
                    service = services[0]
                    schema = (
                        self.load_schema(self.schemas_path, service)
                        if FLAGS.delexicalize
                        else None
                    )
                    ...
                    writer.writerow(
                        [structured_data, text, metadata, dialog_id, turn_id]
                    )
                    turn_id += 1
        ...
        for encoding_scheme in ["naive", "schema_guided", "t2g2"]:
            ...
            if split == "train":
                create_fewshot_splits(
                    dialogs, data_processor, output_dir, encoding_scheme
                )
